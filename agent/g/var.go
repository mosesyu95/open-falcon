...
var (
	OmClient *SingleConnRpcClient
)

func InitOmRpcClients() {
	if Config().Omserver.Enabled {
		OmClient = &SingleConnRpcClient{
			RpcServer: Config().Omserver.Addr,
			Timeout:   time.Duration(Config().Omserver.Timeout) * time.Millisecond,
		}
	}
}
